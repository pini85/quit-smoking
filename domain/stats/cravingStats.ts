...
function nextLocalWeek(d: Date): Date {
...
export function weeklyCounts(
  ...
  while (cursor.getTime() <= to.getTime()) {
    ...
    cursor = nextLocalWeek(cursor);
  }
  return result;
}

export function longestCravingFreeGapMs(s: CravingSession[], quitAt: Date, now: Date): number {
  const times = [quitAt.getTime(), ...s.map((c) => new Date(c.startedAt).getTime()), now.getTime()].sort(
    (a, b) => a - b
  );
  let maxGap = 0;
...
/** null unless now >= quitAt + 14 days (the before-vs-now gate). */
export function firstWeekVsThisWeek(
  ...
  return {
    ...
    thisWeek: weekStats(s, startOfLocalWeek(now)),
  };
}
